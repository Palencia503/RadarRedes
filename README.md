...
Este es un analizador y radar de espectro digital para redes Wi-Fi (bandas de 2.4 GHz, 5 GHz y 6 GHz) que incluye un recomendador tactico de canales y un visualizador de solapamiento fisico de frecuencia. El proyecto corre de forma local e interactiva a traves de una interfaz web cyberpunk.

## Caracteristicas

- **Radar Pasivo en Tiempo Real**: Escanea el aire en busca de redes en todas las frecuencias soportadas por tus antenas y posiciona graficamente las seniales en un radar circular con barrido de fosforo activo.
...
- `radar.py`: Script de consola que ejecuta el escaneo nativo en Windows (netsh) o Linux (nmcli) y muestra un analisis rapido del espectro.
- `gui.py`: Servidor HTTP ligero en Python compatible con Windows y Linux que expone una API REST (`/api/scan`) y sirve los archivos de la interfaz grafica.
...
1. Sistema Operativo: Windows (netsh wlan) o Linux (NetworkManager / nmcli).
